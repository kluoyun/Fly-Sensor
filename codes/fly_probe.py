...
class FlyprobeEndstopWrapper:
    def __init__(self, config):
        ...
        self.add_config_cmd = self.fly_probe_send_cmd = self.fly_probe_calibration_cmd = None
        gcode = self.printer.lookup_object('gcode')
        gcode.register_command("GET_FLY_PROBE",
                                   self.cmd_GET_FLY_PROBE,
                                   desc=self.cmd_GET_FLY_PROBE_help)
        gcode.register_command("FLY_PROBE_Z0",
                                   self.FLY_PROBE_Z0,
                                   desc=self.cmd_GET_FLY_PROBE_help)
        gcode.register_command("FLY_PROBE_CALIBRATION",
                                   self.cmd_FLY_PROBE_CALIBRATION)
        pin = config.get('zstop_pin')
    ...
    def build_config(self):
        self.mcu.add_config_cmd("config_fly_probe_pin oid=%d probe_pwm_pin=%s calibration_pin=%s"
        ...
        self.fly_probe_send_cmd = self.mcu.lookup_query_command(
            "fly_probe oid=%c", "fly_probe_value oid=%c value=%u",
    ...
    def FLY_PROBE_Z0(self, gcmd):
        self.toolhead = self.printer.lookup_object('toolhead')
        self.toolhead.wait_moves()
        self.fly_probe_calibration_cmd.send([self.oid])
    def cmd_FLY_PROBE_CALIBRATION(self, gcmd):
        self.toolhead = self.printer.lookup_object('toolhead')
        self.gcode = self.printer.lookup_object('gcode')
        self.toolhead.manual_move([None,None,0], 200)
        self.toolhead.wait_moves()
        self.gcode.respond_info("FLY_PROBE_CALIBRATION :START")
        CALIBRATION_TIMES = 0
        while CALIBRATION_TIMES < 59:
            self.toolhead.manual_move([None,None,CALIBRATION_TIMES*0.1], 200)
            self.toolhead.wait_moves()
            self.fly_probe_calibration_cmd.send([self.oid])
            CALIBRATION_TIMES += 1
        self.gcode.respond_info("FLY_PROBE_CALIBRATION :END")
